...
Instance  Dictionary::Create(const char name[DICTNAMEMAXSIZE], int size) {
	Instance x;
	if (TEST_CREATE_01(strlen(name)))
		throw THROW01;
	else
		strcpy_s(x.name, name);
	x.size = 0;
	if (TEST_CREATE_02(size)) {
		x.maxsize = DICTMAXSIZE;
		throw THROW02;
	}
	else
		x.maxsize = size;
...
void Dictionary::AddEntry(Instance &inst, Entry ed) {
	if (TEST_ADDENTRY_03(inst.size, inst.maxsize))
		throw THROW03;
	for (int i = 0; i <= inst.size; i++)
		if (TEST_ADDENTRY_04(inst.dictionary[i].id, ed.id))
			throw THROW04;
...
void  Dictionary::UpdEntry(Instance &inst, int id, Entry new_ed) {
	for (int i = 0; i < inst.size; i++)
		if (TEST_UPDENTRY_08(id, new_ed.id, inst.dictionary[i].id))
			throw THROW08;
	for (int i = 0; i <= inst.size; i++)
		if (inst.dictionary[i].id == id) {
